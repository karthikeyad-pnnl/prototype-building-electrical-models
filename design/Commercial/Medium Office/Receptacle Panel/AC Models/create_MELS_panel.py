...
def mels_conv_rating(melstype):
    ...
    if melstype == "Laptop":
        conv_watt = 57
        #Median Model - DELL-DA90PE1-00
        conv_data = [0.030757367,0.04979369,0.083605046,1.061566026]
    
    elif melstype == "Display":
        conv_watt = 127
        #Median Model - RECOM-REDIN240
        conv_data = [0.029810076,0.01181738,0.060621441,0]
...
Excel_filename = "mels_excel.xlsx"
...
for ckts in ckt_lp:
    ...
    mline0 = ("  HPF.Cables.NEC_CableModel cable_rcpt_ckt" + nw_vals + 
    ...
    eline0 = ("  connect(cable_rcpt_ckt" + nw_vals + ".pin_n, " + cable_name + ".pin_p);\n\n")
    ...
    for x in range(0, row_cnt):
        ...
        [conv_alpha,conv_beta,conv_gamma,conv_stndby] = conv_vals[1]
        
        line3a = (",P_stby=" + str(conv_stndby) + ",alpha=" + str(conv_alpha) 
                  + ",beta=" + str(conv_beta) + ",gamma=" + str(conv_gamma))
        ...
        mline3 = ("  HPF.PowerConverters.SinglePhase.ACDC_1pRectifierSimple MEL_Conv_" 
                 + mel_name + "(P_DCmin=2.25, VAC_nom=120,VDC_nom=19.5" 
                 + ",P_nom=" + str(conv_pow) + line3a + ");\n")
        ...
        eline3 = ("  connect(MEL_Conv_" + mel_name + ".pin_p, " + mel_name + ".p);\n")
        eline4 = ("  connect(MEL_Conv_" + mel_name + ".hPin_N, GndAC.pin);\n")
        eline5 = ("  connect(MEL_Conv_" + mel_name + ".pin_n, GndDC.p);\n")
        eline6 = ("  connect(MEL_Conv_" + mel_name + ".hPin_L, cable_rcpt_ckt" + nw_vals + ".pin_p);\n")
...
txt_file_w = "MEL_Panel_L" + str(floor) + str(pha) + ".mo"
f = open(txt_file_w, 'w')
...
with open(txt_file_w, 'r') as file :
  filedata = file.read()
nm = "MEL_Panel_L" + str(floor) + str(pha)
